...
class PacketDecoder:
    ...
    #_wave_packet_unpacker = struct.Struct()
    _instance = None
    ...
    def buildNumericObject(self, header, data):
        ...
        peakV, diasV, meanV, emboliRate, x, x) = self._numeric_packet_unpacker.unpack(data)
        
        formatted_packet = {'Time':timeStamp, 'Channel':CH, 'VesselLabel': str(vessel),'Depth':depth,'Power':power, 'SampleRate':sample, 'EmboliCount':emboliCount, 'PeakVelocity':peakV, 'DiastolicVelocity':diasV, 'MeanVelocity':meanV,'EmboliRate':emboliRate}
        
        return formatted_packet
    
    def buildEventObject(self, header, data):
        self._event_packet_unpacker = struct.Struct(str(len(data))+'c')
        return {'EventMessage:':''.join(self._event_packet_unpacker.unpack(data))}
